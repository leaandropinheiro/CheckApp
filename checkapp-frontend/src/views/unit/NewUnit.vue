...
export default {
  ...
  async mounted() {
    ...
    this.unitImage = await Services.Unit.getUnitImage(5);
    console.log("👉 this.unitImage => ", this.unitImage);
  },
  ...
  methods: {
    ...
    async handleCreateUnit() {
      ...
      const unitResponse = await this.createUnit({
        name: this.name,
        region: this.region,
      ...
      if (unitResponse && this.imageFile) {
        const imageUrl = await Services.Unit.uploadUnitImage(
          this.imageFile,
          unitResponse.id
        );
        this.unitImage = imageUrl;
